...
# -*- coding: utf-8 -*-
"""
signal_snapshot_writer.py — Запись снапшотов спредов по всем парам каждые 0.3 секунды.

Для каждой пары (spot_exch, fut_exch, symbol) из dictionaries/combination/
каждые SNAPSHOT_WRITE_INTERVAL секунд читает из Redis актуальные цены,
вычисляет спред и дописывает строку в CSV-файл.
...
Формат строки (без заголовка после первой строки):
...
class FileHandleManager:
    ...
    def get(self, spot_exch: str, fut_exch: str, symbol: str) -> Any:
        ...
        if key not in self._handles:
            ...
            file_path = dir_path / f"{spot_exch}__{fut_exch}__{symbol}.csv"
...
async def write_loop(
    ...
    Каждые WRITE_INTERVAL секунд:
      ...
      3. Для каждой пары с валидными данными вычисляем спред и пишем строку.
    ...
    while True:
        ...
        for i, (spot_exch, fut_exch, sym) in enumerate(pairs):
            ...
            try:
                ...
                log.write({
                    ...
                    "ts":    _now_iso(),
                    "phase": "file_write",
                    "pair":  f"{spot_exch}__{fut_exch}__{sym}",
                    "error": err,
                })
        ...
        stats.record_cycle(pipeline_ms, write_ms, cycle_ms, written, skipped, overrun)
...
async def snapshot_loop(
    ...
    while True:
        ...
        print(
            ...
            f"skip={stats.skipped_total} err={stats.errors_total} "
            f"overrun={stats.overruns_total}",
            flush=True,
        )
