...
HISTORY_LOOKBACK_SECONDS = float(os.getenv("HISTORY_LOOKBACK_SECONDS", "3600"))  # данные из Redis за последний час
...
async def fetch_history_rows(
    ...
    """
    Читает историю цен из Redis за последние lookback_s секунд.

    Данные коллекторов хранятся в ключах:
        md:hist:{exchange}:{market}:{symbol}:{chunk_id}   (Redis List)
    Каждый элемент: "{ts:.3f},{bid},{ask}"  (ts в секундах)
    Чанки описаны в Hash md:chunks:config, поле "{exchange}:{market}".

    Алгоритм мержа:
        Для каждой точки spot-истории берём последнее известное bid фьючерса
        (forward-fill) и вычисляем спред.

    Возвращает список CSV-строк (без заголовка) в хронологическом порядке.
    """
    now    = time.time()
    cutoff = now - lookback_s

    # 1. Читаем конфиг чанков ─────────────────────────────────────────────────
    try:
        config_raw: Dict[str, str] = await redis_client.hgetall("md:chunks:config")
    except Exception:
        return []

    def _active_chunks(exch: str, mkt: str) -> List[int]:
        raw = config_raw.get(f"{exch}:{mkt}")
        if not raw:
            return []
        try:
            return json.loads(raw).get("active_chunks", [])
        except Exception:
            return []

    spot_chunk_ids = _active_chunks(spot_exch, "spot")
    fut_chunk_ids  = _active_chunks(fut_exch,  "futures")

    if not spot_chunk_ids or not fut_chunk_ids:
        return []

    # 2. Читаем все чанки одним pipeline ──────────────────────────────────────
    spot_keys = [f"md:hist:{spot_exch}:spot:{sym}:{cid}"     for cid in spot_chunk_ids]
    fut_keys  = [f"md:hist:{fut_exch}:futures:{sym}:{cid}"   for cid in fut_chunk_ids]
    ...
    try:
        results: List[List[str]] = await pipe.execute()
    except Exception:
        return []

    n_spot = len(spot_keys)

    # 3. Парсим spot-историю ───────────────────────────────────────────────────
    # lrange 0 -1 при rpush → порядок старый→новый
    spot_history: List[Tuple[float, float, float]] = []   # (ts_s, bid, ask)
    for entries in results[:n_spot]:
        for e in entries:
            try:
                parts = e.split(",")
                ts_s  = float(parts[0])
                if ts_s < cutoff:
                    continue
                spot_history.append((ts_s, float(parts[1]), float(parts[2])))
            except Exception:
                pass

    # 4. Парсим futures-историю ────────────────────────────────────────────────
    fut_history: List[Tuple[float, float, float]] = []    # (ts_s, bid, ask)
    for entries in results[n_spot:]:
        for e in entries:
            try:
                parts = e.split(",")
                ...
                if ts_s < cutoff:
                    continue
                fut_history.append((ts_s, float(parts[1]), float(parts[2])))
            except Exception:
                pass

    if not spot_history or not fut_history:
        return []

    # 5. Сортируем (на случай если чанки шли не по порядку) ───────────────────
    spot_history.sort(key=lambda x: x[0])
    fut_history.sort(key=lambda x: x[0])

    # 6. Мерж: для каждой spot-точки — последний известный fut_bid ─────────────
    rows: List[str] = []
    fut_idx                = 0
    last_fut_bid: Optional[float] = None

    for spot_ts, _spot_bid, spot_ask in spot_history:
        # двигаем указатель фьючерсов вперёд до текущей spot-точки
        while fut_idx < len(fut_history) and fut_history[fut_idx][0] <= spot_ts:
            last_fut_bid = fut_history[fut_idx][1]
            fut_idx += 1

        if last_fut_bid is None or spot_ask <= 0:
            continue

        spread_pct = (last_fut_bid - spot_ask) / spot_ask * 100
        ts_ms      = int(spot_ts * 1000)
        rows.append(
            f"{spot_exch},{fut_exch},{sym},"
            f"{spot_ask},{last_fut_bid},"
...
async def write_loop(
    ...
    while True:
        ...
        for ckey, ts_str in raw_active.items():
            ...
            # ── Вставляем историю за последний час в начало снапшота ──────────
            try:
                history_rows = await fetch_history_rows(
                    redis_client, spot_exch, fut_exch, sym,
                    HISTORY_LOOKBACK_SECONDS,
                )
                if history_rows:
                    fh = fhm.get(spot_exch, fut_exch, sym)
                    ...
                    log.write({
                        ...
                        "lookback_s": HISTORY_LOOKBACK_SECONDS,
                    })
                    print(
                        f"[{SCRIPT_NAME}] history_prepended {spot_exch}→{fut_exch} {sym} "
                        f"rows={len(history_rows)} lookback={HISTORY_LOOKBACK_SECONDS:.0f}s",
                        flush=True,
                    )
                else:
                    log.write({
    ...
    while True:
        ...
        snap: Dict[str, Any] = {
            ...
            "config": {
                ...
                "history_lookback_s":    HISTORY_LOOKBACK_SECONDS,
